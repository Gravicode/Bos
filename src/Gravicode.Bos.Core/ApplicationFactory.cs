    ...
    public static class ApplicationFactory
    {
        public static async Task<ApplicationBuilder> CreateAsync(OutputContext output, FileInfo source, string? framework = null, ApplicationFactoryFilter? filter = null)
        {
            ...
            while (queue.TryDequeue(out var item))
            {
                ...
                foreach (var line in msbuildEvaluationOutput)
                {
                    ...
                    if (trimmed.StartsWith("Gravicode.Bos metadata: "))
                    {
                    ...
                    else if (trimmed.StartsWith("Gravicode.Bos cross-targeting project: "))
                    {
                ...
                if (multiTFMProjects.Any())
                {
                    ...
                    foreach (var line in multiTFMEvaluationOutput)
                    {
                        ...
                        else if (trimmed.StartsWith("Gravicode.Bos cross-targeting project: "))
                        {
                            var values = line.Split(':', 2);
                            var projectName = values[1].Trim();
                            throw new CommandException($"Unable to run {projectName}. Your project targets multiple frameworks. Specify which framework to run using '--framework' or a build property in bos.yaml.");
                        }
                    }
                }
                ...
                foreach (var configService in services)
                {
                    ...
                    {
                        // clone to .bos folder
                        var path = configService.CloneDirectory ?? Path.Join(".bos", "deps");
                        ...
                        if (!Directory.Exists(clonePath))
                        {
                            ...
                            {
                                throw new CommandException($"Cannot clone repository {configService.Repository} because git is not installed. Please install git if you'd like to use \"repository\" in bos.yaml.");
                            }
        ...
        private static async Task<ProcessResult> EvaluateProjectsAsync(IEnumerable<ConfigService> projects, string configRoot, OutputContext output)
        {
            ...
            foreach (var project in projects)
            {
                ...
                sb.AppendLine($"        <MicrosoftBos_ProjectServices " +
            ...
            sb.AppendLine($@"    <Target Name=""MicrosoftBos_EvaluateProjects"">");
            sb.AppendLine($@"        <MsBuild Projects=""@(MicrosoftBos_ProjectServices)"" "
                + $@"Properties=""%(BuildProperties);"
                + $@"MicrosoftBos_ProjectName=%(Name)"" "
                + $@"Targets=""MicrosoftBos_GetProjectMetadata"" BuildInParallel=""true"" />");
        ...
        private static ConfigApplication GetNestedConfig(ConfigApplication rootConfig, string? file)
        {
            ...
            {
                throw new CommandException($"Nested configuration must have the same \"name\" in the bos.yaml. Root config: {rootConfig.Source}, nested config: {nestedConfig.Source}");
            }
