...
namespace Bos.DockerCompose
{
    public class DockerComposeParser : IDisposable
    {
        ...
        public ConfigApplication ParseConfigApplication()
        {
            ...
            {
                throw new BosYamlException(ex.Start, "Unable to parse bos.yaml. See inner exception.", ex);
            }
        ...
        public static string GetScalarValue(YamlNode node)
        {
            ...
            {
                throw new BosYamlException(node.Start,
        ...
        public static string GetScalarValue(string key, YamlNode node)
        {
            if (node.NodeType != YamlNodeType.Scalar)
            {
                throw new BosYamlException(node.Start, CoreStrings.FormatExpectedYamlScalar(key));
            }
        ...
        public static void ThrowIfNotYamlSequence(string key, YamlNode node)
        {
            ...
            {
                throw new BosYamlException(node.Start, CoreStrings.FormatExpectedYamlSequence(key));
            }
        }
                ...
                switch (key)
                {
                    case "context":
                        ...
                        foreach (var format in FileFormats)
                        {
                            ...
                            if (projs.Length > 1)
                            {
                                throw new BosYamlException("Multiple proj files found in directory, have only a single proj file in the context directory.");
                            }
                        }
