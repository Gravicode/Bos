...
namespace Bos.Serialization
{
    public static class ConfigServiceParser
    {
            ...
            foreach (var child in yamlMappingNode!.Children)
            {
                ...
                switch (key)
                {
                    ...
                    case "external":
                        ...
                        {
                            throw new BosYamlException(child.Value.Start, CoreStrings.FormatMustBeABoolean(key));
                        }
                    ...
                    case "dockerFileArgs":
                        ...
                        {
                            throw new BosYamlException(child.Value.Start, CoreStrings.FormatExpectedYamlSequence(key));
                        }
                    ...
                    case "replicas":
                        ...
                        {
                            throw new BosYamlException(child.Value.Start, CoreStrings.FormatMustBeAnInteger(key));
                        }

                        if (replicas < 0)
                        {
                            throw new BosYamlException(child.Value.Start, CoreStrings.FormatMustBePositive(key));
                        }
            ...
            foreach (var child in yamlMappingNode.Children)
            {
                ...
                switch (key)
                {
                    ...
                    case "period":
                        ...
                        if (period < 1)
                        {
                            throw new BosYamlException(child.Value.Start, CoreStrings.FormatMustBeGreaterThanZero(key));
                        }
            ...
            foreach (var child in yamlMappingNode.Children)
            {
                ...
                switch (key)
                {
                    ...
                    case "headers":
                        ...
                        {
                            throw new BosYamlException(child.Value.Start, CoreStrings.FormatExpectedYamlSequence("headers"));
                        }
        ...
        private static void HandleServiceProbeHttpHeader(YamlMappingNode yamlMappingNode, List<KeyValuePair<string, object>> headers)
        {
            ...
            if (name is null)
            {
                throw new BosYamlException(yamlMappingNode.Start, CoreStrings.FormatExpectedYamlScalar("name"));
            }
            else if (value is null)
            {
                throw new BosYamlException(yamlMappingNode.Start, CoreStrings.FormatExpectedYamlScalar("value"));
            }
            ...
            foreach (var child in yamlSequenceNode.Children)
            {
                ...
                switch (child)
                {
                    ...
                    case YamlScalarNode childScalarNode:
                        ...
                        break;
                    default:
                        throw new BosYamlException(child.Start, CoreStrings.FormatUnexpectedTypes($"\"{YamlNodeType.Mapping.ToString()}\", \"{YamlNodeType.Scalar.ToString()}\"", child.NodeType.ToString()));
                }
            ...
            foreach (var child in yamlSequenceNode.Children)
            {
                switch (child)
                {
                    case YamlScalarNode childScalarNode:
                        ...
                        if (!envFile.Exists)
                            throw new BosYamlException(child.Start, CoreStrings.FormatPathNotFound(envFile.FullName));
                        HandleServiceEnvFile(childScalarNode, File.ReadAllLines(envFile.FullName), configuration);
                        break;
                    default:
                        throw new BosYamlException(child.Start, CoreStrings.FormatUnexpectedType(YamlNodeType.Scalar.ToString(), child.NodeType.ToString()));
                }
            }
        }
        ...
        private static void HandleServiceEnvFile(YamlScalarNode yamlScalarNode, string[] envLines, List<ConfigConfigurationSource> configuration)
        {
            foreach (var line in envLines)
            {
                ...
                if (keyValueSeparator == -1)
                    throw new BosYamlException(yamlScalarNode.Start, CoreStrings.FormatExpectedEnvironmentVariableValue(lineTrim));
