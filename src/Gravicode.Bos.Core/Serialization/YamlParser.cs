    ...
    public class YamlParser : IDisposable
    {
        ...
        public ConfigApplication ParseConfigApplication()
        {
            ...
            catch (YamlException ex)
            {
                if (_fileInfo != null)
                {
                    throw new BosYamlException(ex.Start, $"Unable to parse '{_fileInfo.Name}'. See inner exception.", ex, _fileInfo);
                }

                throw new BosYamlException(ex.Start, $"Unable to parse YAML.  See inner exception.", ex);
            }
        ...
        public static Dictionary<string, object> GetDictionary(YamlNode node)
        {
            ...
            foreach (var mapping in (YamlMappingNode)node)
            {
                ...
                dictionary[key] = mapping.Value.NodeType switch
                {
                    ...
                    _ => throw new BosYamlException(mapping.Value.Start,
