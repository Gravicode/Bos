    ...
    public static class ConfigFactory
    {
        ...
        private static ConfigApplication FromSolution(FileInfo file)
        {
            ...
            foreach (var projectFile in ProjectReader.EnumerateProjects(file))
            {
                ...
                // runnable. This will only apply in the case where bos is being used against a solution
                // like `bos init` or `bos run` without a `bos.yaml`.
                //
        ...
        private static ConfigApplication FromBosYaml(FileInfo file)
        {
