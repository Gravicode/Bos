        ...
        public override async Task ExecuteAsync(OutputContext output, ApplicationBuilder application, IngressBuilder ingress)
        {
            ...
            if (!Interactive)
            {
                throw new CommandException(
                    ...
                    $"see our documentation on ingress: https://aka.ms/bos/ingress");
            }

            output.WriteAlwaysLine(
                "Bos can deploy the ingress-nginx controller for you. This will be a basic deployment suitable for " +
                ...
                "See: https://aka.ms/bos/ingress for documentation.");
            ...
            else
            {
                ...
                output.WriteCommandLine("kubectl", $"apply -f \"https://aka.ms/bos/ingress/deploy\"");
                var capture = output.Capture();
                var exitCode = await ProcessUtil.ExecuteAsync(
                    $"kubectl",
                    $"apply -f \"https://aka.ms/bos/ingress/deploy\"",
                    System.Environment.CurrentDirectory);
