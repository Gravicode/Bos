...
namespace Gravicode.Bos
{
    ...
    {
        // For layering reasons this has to live in the `bos` project. We don't want to leak
        // the extensions themselves into Bos.Core.
        public static async Task ProcessExtensionsAsync(this ApplicationBuilder application, HostOptions? options, OutputContext output, ExtensionContext.OperationKind operation)
        {
            ...
            {
                if (!Gravicode.Bos.Extensions. WellKnownExtensions.Extensions.TryGetValue(extensionConfig.Name, out var extension))
                {
        ...
        public static Bos.Hosting.Model.EnvironmentVariable ToHostingEnvironmentVariable(this EnvironmentVariableBuilder builder)
        {
            var env = new Bos.Hosting.Model.EnvironmentVariable(builder.Name);
            env.Value = builder.Value;
            if (builder.Source != null)
            {
                env.Source = new Bos.Hosting.Model.EnvironmentVariableSource(builder.Source.Service, builder.Source.Binding);
                env.Source.Kind = (Bos.Hosting.Model.EnvironmentVariableSource.SourceKind)(int)builder.Source.Kind;
            }
