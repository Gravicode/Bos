...
{
    public class BosDeserializationValidationTests
    {
        [Fact]
    ...
    bindings:
      ...
      - port: 8080
        protocol: http";
            ...
            var exception = Assert.Throws<BosYamlException>(() => app.Validate());
    ...
    bindings:
      - port: 8080
        protocol: http";
            ...
            using var parser = new YamlParser(input);
            var app = parser.ParseConfigApplication();
            Assert.Throws<BosYamlException>(() => app.Validate());
        }
...
flimflam";
            ...
            catch (BosYamlException e)
            {
                Assert.StartsWith("Error parsing 'foobar.yml': (2, 1): Unexpected node type in the bos configuration file.", e.Message);
            }
        }
