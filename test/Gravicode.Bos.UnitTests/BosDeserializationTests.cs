...
{
    public class BosDeserializationTests
    {
        private IDeserializer _deserializer;

        public BosDeserializationTests()
        {
...
ingress:
  - name: ingress
    ...
    replicas: 2";
            ...
            BosAssert.Equal(expected, actual);
        }
...
", setWorkingDirectory ? new FileInfo(Path.Join(Directory.GetCurrentDirectory(), "testassets", "bos.yaml")) : null);
...
services:
    - env_file:
        - ./envfile_c.env
", new FileInfo(Path.Join(Directory.GetCurrentDirectory(), "testassets", "bos.yaml")));

            var exception = Assert.Throws<BosYamlException>(() => parser.ParseConfigApplication());
