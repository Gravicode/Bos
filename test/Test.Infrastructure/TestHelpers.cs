    ...
    public static class TestHelpers
    {
        ...
        public static DirectoryInfo GetTestAssetsDirectory()
        {
            return new DirectoryInfo(Path.Combine(
                TestHelpers.GetSolutionRootDirectory("bos"),
                "test",
                "E2ETest",
                "testassets"));
        }
        ...
        public static TempDirectory CopyTestProjectDirectory(string projectName)
        {
            ...
            // We need to hijack any P2P references to Bos libraries.
            // Test projects must use $(BosLibrariesPath) to find their references.
            var libraryPath = Path.Combine(TestHelpers.GetSolutionRootDirectory("bos"), "src");
...
<Project>
  <PropertyGroup>
    <BosLibrariesPath>{libraryPath}</BosLibrariesPath>
  </PropertyGroup>
</Project>");
        ...
        public static async Task<bool> DoOperationAndWaitForReplicasToChangeState(BosHost host, ReplicaState desiredState, int n, HashSet<string> toChange, HashSet<string> rest, Func<ReplicaEvent, string> entitySelector, TimeSpan waitUntilSuccess, Func<BosHost, Task> operation)
        {
        ...
        public static async Task<bool> DoOperationAndWaitForReplicasToRestart(BosHost host, HashSet<string> toRestart, HashSet<string> rest, Func<ReplicaEvent, string> entitySelector, TimeSpan waitUntilSuccess, Func<BosHost, Task> operation)
        {
        ...
        public static Task<bool> DoOperationAndWaitForReplicasToChangeState(BosHost host, ReplicaState desiredState, int n, HashSet<string> toChange, HashSet<string> rest, TimeSpan waitUntilSuccess, Func<BosHost, Task> operation)
        ...
        public static Task<bool> DoOperationAndWaitForReplicasToRestart(BosHost host, HashSet<string> toRestart, HashSet<string> rest, TimeSpan waitUntilSuccess, Func<BosHost, Task> operation)
            => DoOperationAndWaitForReplicasToRestart(host, toRestart, rest, ev => ev.Replica.Name, waitUntilSuccess, operation);

        public static async Task StartHostAndWaitForReplicasToStart(BosHost host, string[] services = null, ReplicaState desiredState = ReplicaState.Started)
        {
            if (services == null)
            {
        ...
        public static async Task PurgeHostAndWaitForGivenReplicasToStop(BosHost host, string[] replicas)
        {
            static async Task Purge(BosHost host)
            {
