...
namespace Test.Infrastructure
{
    public class EqualityYamlNodeVisitor
    {
        ...
        private void VisitInternal(YamlNode node, YamlNode otherNode)
        {
            ...
            {
                throw new BosYamlException($"Node types differ, Expected: {node.NodeType} at ({node.Start.Line}, {node.Start.Column}). " +
                    $"Actual: {node.NodeType} at ({otherNode.Start.Line}, {otherNode.Start.Column}).");
            }

            if (node.Tag.Equals(otherNode.Tag) == false)
            {
                throw new BosYamlException($"Expected tags to be equal. " +
        ...
        public void VisitSequence(YamlSequenceNode node, YamlSequenceNode otherNode)
        {
            ...
            {
                throw new BosYamlException($"Extra children in expected yaml sequence, Expected: ({node.Start.Line}, {node.Start.Column}). Actual: ({otherNode.Start.Line}, {otherNode.Start.Column})");
            }
            ...
            if (node.Children.Count < otherNode.Children.Count)
            {
                throw new BosYamlException($"Extra children in actual yaml sequence, Expected: ({node.Start.Line}, {node.Start.Column}). Actual: ({otherNode.Start.Line}, {otherNode.Start.Column}).");
            }
        }
        ...
        public void VisitMapping(YamlMappingNode node, YamlMappingNode otherNode)
        {
            foreach (var child in node.Children)
            {
                ...
                {
                    throw new BosYamlException($"YamlMapping missing node, difference starting at Expected: ({node.Start.Line}, {node.Start.Column}). " +
...
{string.Join(Environment.NewLine, otherNode.Children.Select(kvp => $"      " + kvp.Key.ToString() + ": ..."))}");
        ...
        public void VisitScalar(YamlScalarNode node, YamlScalarNode otherNode)
        {
            ...
            {
                throw new BosYamlException($"Scalar nodes have different values." + Environment.NewLine +
