...
services:
- name: vote
  include: vote/bos.yaml
- name: results
  include: results/bos.yaml";
            var yamlFile = Path.Combine(projectDirectory.DirectoryPath, "bos.yaml");
            await File.WriteAllTextAsync(yamlFile, content);
...
- name: redis
  image: redis2";
            var yamlFile = Path.Combine(projectDirectory.DirectoryPath, "results", "bos.yaml");
            await File.WriteAllTextAsync(yamlFile, content);
        ...
        public async Task WrongProjectPathProducesCorrectErrorMessage()
        {
            ...
            var projectFile = new FileInfo(Path.Combine(projectDirectory.DirectoryPath, "bos-wrong-projectpath.yaml"));
            var outputContext = new OutputContext(_sink, Verbosity.Debug);
        ...
        public async Task TargetFrameworkFromCliArgs()
        {
            ...
            var yamlFile = Path.Combine(projectDirectory.DirectoryPath, "bos-no-buildproperties.yaml");
        ...
        {
            using var projectDirectory = TestHelpers.CopyTestProjectDirectory(Path.Combine("multi-targetframeworks"));
            var yamlFile = Path.Combine(projectDirectory.DirectoryPath, "bos-with-netcoreapp21.yaml");
