...
using Gravicode.Bos.Hosting.Model.V1;
...
namespace E2ETest
{
    public class BosRunTests
    {
        private readonly ITestOutputHelper _output;
        private readonly TestOutputLogEventSink _sink;
        private readonly JsonSerializerOptions _options;

        public BosRunTests(ITestOutputHelper output)
        {
            _output = output;
...
- name: frontend
  project: frontend/frontend.csproj";
        ...
        public async Task FrontendBackendRunTestWithDockerAndBuildConfigurationAsProperty(string buildConfiguration)
        {
            ...
            var projectFile = new FileInfo(Path.Combine(projectDirectory.DirectoryPath, $"bos-{buildConfiguration.ToLower()}-configuration.yaml"));
            var outputContext = new OutputContext(_sink, Verbosity.Debug);
            var application = await ApplicationFactory.CreateAsync(outputContext, projectFile);
        ...
        public async Task DockerNamedVolumeTest()
        {
            ...
            var volumeName = "bos_docker_volumes_test" + Guid.NewGuid().ToString().Substring(0, 10);
        ...
        public async Task DockerNetworkAssignmentTest()
        {
            ...
            var dockerNetwork = "bos_docker_network_test" + Guid.NewGuid().ToString().Substring(0, 10);
        ...
        public async Task IngressStaticFilesTest()
        {
            ...
            var projectFile = new FileInfo(Path.Combine(projectDirectory.DirectoryPath, "bos-ui.yaml"));
            var outputContext = new OutputContext(_sink, Verbosity.Debug);
            var application = await ApplicationFactory.CreateAsync(outputContext, projectFile);
        ...
        private async Task TestIngressIP(string ipSetting, IEnumerable<IPAddress> mustAnswer, IEnumerable<IPAddress> mustFail)
        {
            ...
            using var projectDirectory = CopyTestProjectDirectory("apps-with-ingress");
            var projectFile = new FileInfo(Path.Combine(projectDirectory.DirectoryPath, "bos-ip_test.yaml"));
        ...
        {
            // https://github.com/dotnet/bos/issues/428
        ...
        public async Task NullDebugTargetsDoesNotThrow()
        {
            ...
            var outputContext = new OutputContext(_sink, Verbosity.Debug);
            var application = await ApplicationFactory.CreateAsync(outputContext, projectFile);
            await using var host = new BosHost(application.ToHostingApplication(), new HostOptions())
            {
                Sink = _sink,
            };
        ...
        public async Task MultiRepo_Works()
        {
            ...
            var projectFile = new FileInfo(Path.Combine(projectDirectory.DirectoryPath, "vote", "bos.yaml"));
...
services:
- name: vote
  repository: https://github.com/jkotalik/BosMultiRepoVoting
- name: results
  repository: https://github.com/jkotalik/BosMultiRepoResults";

            var yamlFile = Path.Combine(projectDirectory.DirectoryPath, "bos.yaml");
...
- name: frontend
  project: frontend/frontend.csproj";
        ...
        public async Task RunExplicitYamlMultipleTargetFrameworksTest()
        {
            ...
            var projectFile = new FileInfo(Path.Combine(projectDirectory.DirectoryPath, "bos-with-netcoreapp31.yaml"));
            var outputContext = new OutputContext(_sink, Verbosity.Debug);
            var application = await ApplicationFactory.CreateAsync(outputContext, projectFile);
        ...
        public async Task RunWithArgsMultipleTargetFrameworksTest()
        {
            ...
            var projectFile = new FileInfo(Path.Combine(projectDirectory.DirectoryPath, "bos-no-buildproperties.yaml"));
            var outputContext = new OutputContext(_sink, Verbosity.Debug);
        ...
        [InlineData("non-standard-dashboard-port", "mcr.microsoft.com/dotnet/aspnet:6.0", 8005)]
        [InlineData("non-standard-dashboard-port-5.0", "mcr.microsoft.com/dotnet/aspnet:5.0", 8006)]
        public async Task RunCliPortOverridesYamlDashboardPort(string projectName, string baseImage, int bosYamlDashboardPort)
        {
            ...
            Assert.Equal(bosYamlDashboardPort, application.DashboardPort);
            ...
            {
                // Make sure the dashboard is running on the expected port passed from the CLI, not the value from bos.yaml
                Assert.Equal(cliDashboardPort, uri.Port);
                Assert.NotEqual(bosYamlDashboardPort, uri.Port);
        ...
        {
            await using var host = new BosHost(application.ToHostingApplication(), options)
            {
                Sink = _sink,
            };
